...
export const AddCategoryView = ({ onComplete }: { onComplete?: () => void }) => {
  const [form, setForm] = useState({ name: "", slug: "", description: "", icon: "📦", order: 1, active: true });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newRef = doc(db, "categories", slug);
      await withTimeout(setDoc(newRef, { ...cleanPayload, id: newRef.id, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge: true }));
      toast.success("Kategori baru ditambahkan");
      
      setForm({ name: "", slug: "", description: "", icon: "📦", order: 1, active: true });
